...
int getargs(char *cmd, char **argv); // 사용자 입력 명령어를 공백 단위로 분리
int handle_builtin_commands(char **argv); // 내장 명령어 처리 함수
void handle_sigint(int sig);  // SIGINT(Ctrl-C) 시그널 처리
void handle_sigquit(int sig); // SIGQUIT(Ctrl-\) 시그널 처리
void handle_sigtstp(int sig); // SIGTSTP(Ctrl-Z) 시그널 처리
